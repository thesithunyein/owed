...
//!
//! Used so the Merkle scheme is byte-identical across Rust, TypeScript and the
//! on-chain program without pulling external crates into the core.
...
impl Sha256 {
    ...
    pub fn finalize(mut self) -> [u8; 32] {
        ...
        // Length block: bypass the bitlen accounting with a direct write.
    ...
    fn compress(&mut self, block: &[u8; 64]) {
        ...
        for (s, v) in self.state.iter_mut().zip(deltas) {
            *s = s.wrapping_add(v);
        }
    }
}
