...
> (classified at 2026-09-26 15:12 UTC); 4 are off by 100% or more, and 2 by a full 10x.
...
CI runs the full verification spine on every push: Rust tests, 138 keeper tests,
...
| Median time since the stored field diverged | **29 days** |
| Longest divergence | **353 days** (`GMEx`) |
...
is fine says so. The full findings are collapsed behind one disclosure rather than
filling the first screen, and a shared link carries its token (`#t=AAPLx`). All of
it re-classified against your clock on load. The method and the evidence are
**not** on the page: they are in this file, because a visitor is here to use the
app and a skeptic is here to falsify a number. The table below is the bridge.
...
## From a surface to its evidence

Every number the app shows is derived from a committed artifact, and the point of
publishing the artifact is that a reader can check the number rather than trust
the page. This table is the index: the left column is what you see at
<https://owed.sithunyein.com>, the right column is where that surface comes from
and what reproduces it. Every path below is resolved by a test, so this index
cannot quietly rot into a list of files that no longer exist.
...
| `/` - findings registry | every stale multiplier, worst first | rows in `feed/owed-risk.json`; source scans `keeper/data/xstocks-scan.json` and `keeper/data/prestocks-scan.json`; rebuilt by `scripts/risk-feed.mjs` |
| `/board` and `/board?issuer=prestocks` | all 933 mints, both issuers, stored vs applied, issuer controls | the two scan files above; classifier in `keeper/src/scaled.mjs`; the PreStocks fixture in `core/tests/golden_vectors.rs` |
| `/integrate` - JavaScript | `getEffectiveMultiplier(mint)`, zero dependencies, no key | `sdk/owed.mjs`, `sdk/example.mjs`, `sdk/test` |
| `/integrate` - Rust / CPI | the same reader as a program dependency | `core/src/multiplier.rs`, `core/tests/golden_vectors.rs` |
| `/integrate` - devnet table | a 15-step settlement run against a deployed program | `docs/devnet-settlement-2026-09-22.json`; rerun by `.github/workflows/ci.yml` |
| `/feed/owed-risk.json` | the integration contract itself | generated by `scripts/risk-feed.mjs`, described by `feed/schema.json`, checked by `keeper/test/feed.test.mjs` |
| `/feed/alerts.json` | the alert lane as an append-only document | written by `scripts/alert-digest.mjs` |
<!-- owed:provenance:end -->

Two rows are worth opening first. `keeper/data/conformance.json` is the run that
compares our reader against the runtime's own scaled supply across the official
xStocks set, which is the difference between a reader that is argued for and one
that is measured. And `feed/schema.json` is what keeps the feed from being a JSON
blob whose meaning lives in a paragraph somewhere else.
...
│   └── test/              #   138 tests incl. build-integrity guards on the pages,
...
| `keeper/` TS | ✅ 138 tests - trap logic, scaled classifier pinned to real account shapes, feed contract, page build integrity (including the static-fallback, alert-strip, provenance, plain-language, hero-restraint, control-styling, middle-dot and word-budget guards), Merkle parity, RPC parsing, base58, Pyth lane, 500-holder stress |
